...
POS_TYPE DataPage<KeyType>::write(std::fstream &file) {
    ...
    for (int i = 0; i < capacity; ++i) {
        file.write((char *) &(records[static_cast<unsigned long>(i)]), sizeof(ISAMRecord<KeyType>));
    }
    return pos;
}
...
void DataPage<KeyType>::read(std::fstream &file) {
    ...
    // Read records
    for (int i = 0; i < capacity; ++i) {
        file.read((char *) &(records[static_cast<unsigned long>(i)]), sizeof(ISAMRecord<KeyType>));
    }
}
 ...
 *             |-----------records-------------|   |-----next-----|   |-----count-----|
 * BufferSize = sizeof(ISAMRecord<KeyType> * (N) + sizeof(POS_TYPE) + sizeof(POS_TYPE)
 * (BufferSize - sizeof(POS_TYPE) - sizeof(POS_TYPE)) / sizeof(ISAMRecord<KeyType> = N
 */

template<typename KeyType>
POS_TYPE DataPage<KeyType>::getCapacity() {
//    auto a = (buffer::get_buffer_size() - sizeof(POS_TYPE)*2) / sizeof(ISAMRecord<KeyType>);
...
template<typename KeyType>
void DataPage<KeyType>::setRecord(ISAMRecord<KeyType> record,
                                  int64_t pos) {
    try {
...
ISAMRecord<KeyType> &DataPage<KeyType>::getRecord(int64_t pos) {
    try {
        ...
        throw (std::runtime_error(
                "Cannot access in position: " + std::to_string(pos)));
    }
}
...
template<typename KeyType>
void DataPage<KeyType>::setNext(POS_TYPE n) {
    DataPage::next = n;
}
...
void DataPage<KeyType>::remove(int64_t i) {
    ...
    if (i == getCount() - static_cast<POS_TYPE>(1)) {
        ...
        records[static_cast<unsigned long>(j - static_cast<POS_TYPE>(1))] = ISAMRecord<KeyType>();
    }
    count = count - static_cast<POS_TYPE >(1);
}

template<typename KeyType>
ISAMRecord<KeyType> DataPage<KeyType>::last() {
    return records[static_cast<unsigned long>(count-static_cast<POS_TYPE>(1))];
}


template<typename KeyType>
void DataPage<KeyType>::clearRecord(POS_TYPE pos) {
