...
ISAM<KeyType>::ISAM(std::string _table_name, std::string _attribute_name,
    ...
    this->idx_filename =
        _directory + "/" + _table_name + "_" + _attribute_name + "_" + this->index_name + "_indexPages.bin";

    this->dt_filename =
        _directory + "/" + _table_name + "_" + _attribute_name + "_" + this->index_name + "_dataPages.bin";
...
template<typename KeyType>
void ISAM<KeyType>::close_files() const {
    idx_file.close();
    dt_file.close();
    dup_file.close();
}
...
void ISAM<KeyType>::build(std::vector<ISAMRecord<KeyType>> &data) const {
    ...
    auto compare = [](ISAMRecord<KeyType> &a,
                      ISAMRecord<KeyType> &b) {
        return a.key < b.key;
    };
...
template<typename KeyType>
void ISAM<KeyType>::_search(POS_TYPE node_pos, KeyType key, std::vector<POS_TYPE> &result) const {
    // Read the node
...
template<typename KeyType>
void ISAM<KeyType>::_insert(POS_TYPE node_pos, ISAMRecord<KeyType> record, std::vector<POS_TYPE> &result) const {
    // Read the node
    idx_file.seekg(node_pos, std::ios::beg);
    IndexPage<KeyType> node;
    node.read(idx_file);
...
template<typename KeyType>
void ISAM<KeyType>::splitAndInsert(DataPage<KeyType>& page, ISAMRecord<KeyType> record) const {
    POS_TYPE index = page.getCount() - static_cast<POS_TYPE>(1); // Position where the record should be inserted
...
template<typename KeyType>
Response ISAM<KeyType>::range_search(Data<KeyType> beg, Data<KeyType> end) const {
    if (root == -1)
        throw std::runtime_error("No indexed data in ISAM, please build before");
...
void ISAM<KeyType>::_range_search(POS_TYPE node_pos, KeyType beg, KeyType end,
    ...
    if (node.getIsLeaf()) {
        ...
        auto last_pos = static_cast<POS_TYPE>(
            std::ceil(pow(static_cast<double>(M + static_cast<POS_TYPE>(1)), ISAM_LEVELS) * static_cast<double>(DataPage<KeyType>::size_of())));
...
template<typename KeyType>
void ISAM<KeyType>::_remove(POS_TYPE node_pos, KeyType key, std::vector<POS_TYPE> &result) const {
    // Read the node
    idx_file.seekg(node_pos, std::ios::beg);
    IndexPage<KeyType> node;
    node.read(idx_file);
    ...
    if (node.getIsLeaf()) {
        ...
        do {
            ...
            if (key <= page.last().key)
            {
                ...
                else if (page.getCount() <= static_cast<POS_TYPE>(std::floor(DataPage<KeyType>::getCapacity()/2)))
                {
                    merge(page);
...
void ISAM<KeyType>::merge(DataPage<KeyType> &page) const {
    ...
    if (page.getCount() + next_page.getCount() <= DataPage<KeyType>::getCapacity()) {
        // page.count = floor(N/2)-1 and  floor(N/2) <= next_page.count <= floor(N/2)+1
        POS_TYPE page_count = page.getCount();
        for (int i = 0; i < next_page.getCount(); ++i)
        {
            page.setRecord(next_page.getRecord(i), page_count + static_cast<POS_TYPE>(i));
        }
...
template<typename KeyType>
void ISAM<KeyType>::insertDuplicate(ISAMRecord<KeyType> &rec, ISAMRecord<KeyType> &rec_dup) const {
    try {
...
template<typename KeyType>
void ISAM<KeyType>::insertDuplicateFile(ISAMRecord<KeyType> &rec, POS_TYPE &duplicate_position) const {
    if (!dup_file.is_open())
...
template<typename KeyType>
void ISAM<KeyType>::getAllRawCurrentRecords(ISAMRecord<KeyType> record, std::vector<POS_TYPE> &records) const {
    if (!dup_file.is_open())
        throw std::runtime_error("Couldn't open duplicatesFile");
    ...
    try {
        ...
        while (record.dup_pos != -1) {
            ...
            dup_file.read(reinterpret_cast<char *>(&record), sizeof(ISAMRecord<KeyType>));
            records.push_back(record.pos);
        }
    } catch (...) {
...
template<typename KeyType>
std::string ISAM<KeyType>::get_table_name() const {
