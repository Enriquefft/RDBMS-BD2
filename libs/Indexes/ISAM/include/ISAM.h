...
class ISAM : public Index::Index<KeyType>{
private:
...
//    ISAM(std::vector<ISAMRecord<KeyType>> &data);
    ...
    POS_TYPE max_records() const {
      return MIN_BULK_INSERT_SIZE::value;
    }
    ...
    Index::Response range_search(Data<KeyType> beg, Data<KeyType> end) const override;
    ...
    std::pair<Index::Response, std::vector<bool>> bulk_insert(const std::vector<std::pair<KeyType, POS_TYPE>>& data) const override {
      Index::Response response;
      response.startTimer();
      std::vector<ISAMRecord<KeyType>> other;
      for (auto& pair : data) other.push_back(ISAMRecord<KeyType>{pair.first, pair.second});
      build(other);
      response.stopTimer();
...
private:
    ...
    void _search(POS_TYPE node_pos, KeyType key, std::vector<POS_TYPE> &result) const;

    void _insert(POS_TYPE node_pos, ISAMRecord<KeyType> record, std::vector<POS_TYPE> &result) const;

    void _range_search(POS_TYPE node_pos, KeyType beg, KeyType end,
                       std::vector<POS_TYPE> &result) const;

    void _remove(POS_TYPE node_pos, KeyType key, std::vector<POS_TYPE> &result) const;

    void insertDuplicate(ISAMRecord<KeyType> &rec, ISAMRecord<KeyType> &rec_dup) const;

    void insertDuplicateFile(ISAMRecord<KeyType> &rec, POS_TYPE &duplicate_position) const;

    void getAllRawCurrentRecords(ISAMRecord<KeyType> record, std::vector<POS_TYPE> &records) const;

    void splitAndInsert(DataPage<KeyType>& page, ISAMRecord<KeyType> record) const;
