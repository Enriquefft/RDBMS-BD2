...
void Index<KEY_TYPE>::getAllRawCurrentRecords(RecordType sir, std::vector<physical_pos>& records) {
...
void Index<KEY_TYPE>::insertDuplicate(FileType& file, RecordType& rec, RecordType& rec_dup){
    try {
...
template<typename KEY_TYPE>
template<typename RecordType>
void Index<KEY_TYPE>::insertDuplicateFile(RecordType& rec, physical_pos& duplicate_position){
    std::fstream duplicatesFile(this->duplicatesFilename, std::ios::in | std::ios::out | std::ios::binary);
    if (!duplicatesFile.is_open()) throw std::runtime_error("Couldn't open duplicatesFile");
...
template<typename KEY_TYPE>
template<typename RecordType>
void Index<KEY_TYPE>::printFile(std::string file_name) {	
    std::fstream file(file_name, std::ios::in | std::ios::out | std::ios::binary);
    if (!file.is_open()) { throw std::runtime_error("Couldn't open file"); }
    RecordType record;
...
template<typename KEY_TYPE>
template<typename FileType, typename RecordType>
void Index<KEY_TYPE>::moveReadRecord(FileType& file, physical_pos& pos, RecordType& record){
    try {
...
template<typename KEY_TYPE>
template<typename FileType, typename RecordType>
void Index<KEY_TYPE>::moveWriteRecord(FileType& file, physical_pos& pos, RecordType& record){
    try {
        file.seekp(pos, std::ios::beg);
        file.write(reinterpret_cast<char*> (&record), sizeof(RecordType));
    } catch (...) {
