...
TEST_F(ElRealTest, selectWithComplexCondition) {
    auto query = "SELECT FROM " + TEST_TABLE + " WHERE id > 5 AND name = 'AARON';";
    auto response = test_request(query);
    EXPECT_EQ(response.code, 200);
}

TEST_F(ElRealTest, selectWithMoreComplexCondition) {
    auto query = "SELECT FROM " + TEST_TABLE + " WHERE id < 12 AND name = 'AARON' OR name = 'JUAQUIN';";
    auto response = test_request(query);
    EXPECT_EQ(response.code, 200);
}

TEST_F(ElRealTest, selectWithEvenMoreComplexCondition) {
    auto query = "SELECT FROM " + TEST_TABLE + " WHERE id > 5 AND name = 'AARON' OR name = 'JUAQUIN' AND id < 12;";
    auto response = test_request(query);
    EXPECT_EQ(response.code, 200);
}
...
TEST_F(ElRealTest, deleteFromTableWithComplexCondition) {
    auto query = "DELETE FROM " + TEST_TABLE + " WHERE id > 5 AND name = 'JUAQUIN' OR name = 'ENRIQUE;";
    auto response = test_request(query);
    EXPECT_EQ(response.code, 200);
}
